...
class DoubleTransmonSystem:
    ...
    def two_qubit_operator(self, qubit1_operator=None,
                           qubit2_operator=None):

        mask = [qp.identity(self._tr1.get_Ns()), qp.identity(
            self._tr2.get_Ns())]
    ...
    def H(self, phi1, phi2):
        H = self.two_qubit_operator(
            qubit1_operator=self._tr1.h_diag_trunc(phi1)) + \
            self.two_qubit_operator(
                qubit2_operator=self._tr2.h_diag_trunc(phi2)) + \
            self.Hint(phi1, phi2)
        return H - H[0, 0]
    ...
    def H_iswap_RF_RWA_td(self, waveform1, waveform2, params, rotations):
        ...
        freq1 = (evals1[1] - evals1[0]) / 2 / np.pi
        Hdr1 = self._tr1.Hdr_RF_RWA(params['drive_amplitude'],
                                    params['start'] - dur1 - 10, dur1,
                                    phi1)
        Huu1 = self._tr1.H_uu(freq1) * (-1)
        H1 = Htd1 + [Hdr1] + [Huu1]
        H1_2q = [
            [self.two_qubit_operator(qubit1_operator=H1[0][0]), H1[0][1]],
        ...
        freq2 = (evals2[1] - evals2[0]) / 2 / np.pi
        Hdr2 = self._tr2.Hdr_RF_RWA(params['drive_amplitude'],
                                    params['start'] - dur2 - 10, dur2,
                                    phi2)
        Huu2 = self._tr2.H_uu(freq2) * (-1)
        H2 = Htd2 + [Hdr2] + [Huu2]
        H2_2q = [
            [self.two_qubit_operator(qubit2_operator=H2[0][0]), H2[0][1]],
        ...
        Hint = self.Hint_RF_RWA(flux1, flux2, (freq2 - freq1) * 2 * np.pi,
                                td=True)
        H = H1_2q + H2_2q + Hint
    ...
    def H_td(self, waveform1, waveform2):
        return [self.Hc()] + self.Hj_td(waveform1, waveform2) + [
            self.Hint()]
    ...
    def H_td_diag_approx_str(self, params, iswap2=True):
        ...
        if (iswap2):
            gap = 120.00
            waveform1_iswap2 = self._tr1.waveform_td(params['phi_offset'],
                                                     params['start'] + gap,
                                                     params['start'] +
                                                     params[
                                                         'duration'] + gap)
            waveform1_zgate2 = self._tr1.waveform_td(params['phiz_offset'],
                                                     params[
                                                         'start'] + 90 + 120,
                                                     params[
                                                         'start'] + 90 + gap +
                                                     params['t_zgate'])
            waveform2_zgate2 = self._tr2.waveform_td(
                params['phi2z_offset'], params['start'] + 90 + gap,
                params['start'] + 90 + gap + params['t_zgate2'])

        waveform1_iswap1 = self._tr1.waveform_td(params['phi_offset'],
                                                 params['start'],
                                                 params['start'] + params[
                                                     'duration'])
        ...
        # waveform1_zgate = 't-t '
        waveform1_zgate1 = self._tr1.waveform_td(params['phiz_offset'],
                                                 params['start'] + 90,
                                                 params['start'] + 90 +
                                                 params['t_zgate'])

        waveform1_zgate = '( ' + waveform1_zgate1 + ' + ' + waveform1_zgate2 + ' )'
        waveform1_sum = '( ' + waveform1_iswap + ' + ' + waveform1_zgate + ' + ' + ' %f' % \
                        params['phi_base_level'] + ' )'
        # waveform2_zgate = 't-t '
        waveform2_zgate1 = self._tr2.waveform_td(params['phi2z_offset'],
                                                 params['start'] + 90,
                                                 params['start'] + 90 +
                                                 params['t_zgate2'])
        ...
        waveform2_sum = '( ' + waveform2_zgate + ' + ' + ' %f' % params[
            'phi2z_base_level'] + ' )'
    ...
    def H_td_diag_approx(self, waveform1, waveform2):
        ...
        return [Hj_td1, Hj_td2,
                self.Hint_RF_RWA_waveform(waveform1, waveform2)]
        # return [self.Hint_RF_RWA_waveform(waveform1, waveform2)]

    def H_diag_approx(self, phi1, phi2):
        return self.two_qubit_operator(
            qubit1_operator=self._tr1.H_diag_trunc_approx(phi1)) + \
               self.two_qubit_operator(
                   qubit2_operator=self._tr2.H_diag_trunc_approx(phi2)) + \
               self.Hint_RF_RWA(0.4770, 0.5)

    def Hc(self):
        return self.two_qubit_operator(
            qubit1_operator=self._tr1.calc_Hc_cb()) + \
               self.two_qubit_operator(
                   qubit2_operator=self._tr2.calc_Hc_cb())

    def Hj(self, phi1, phi2):
        return self.two_qubit_operator(
            qubit1_operator=self._tr1.calc_Hj_cb(phi1)) + \
               self.two_qubit_operator(
                   qubit2_operator=self._tr2.calc_Hj_cb(phi2))
    ...
    def Hint(self, phi1, phi2):
        return self.two_qubit_operator(self._tr1.n(phi1),
                                       self._tr2.n(phi2)) * self._g
    ...
    def Hint_RF_RWA(self, phi1, phi2, offset=0.02, td=False):
        ...
        for i in range(op1.shape[0]):
            for j in range(op1.shape[1]):
                ...
                if i > j:
                    op2[i, j] = 0
        half = self.two_qubit_operator(qp.Qobj(op1), qp.Qobj(op2))
        if td:
            return [[self._g * half, "exp(-1j*(%f)*t)" % offset],
                    [self._g * half.dag(), "exp(1j*(%f)*t)" % offset]]
        else:
            return self._g * (half + half.dag())

    def Hint_RF_RWA_waveform(self, waveform1, waveform2, td=False):
        return [self.Hint_RF_RWA(0, 0.5), np.array(
            [self._tr1._phi_coeff(phi) ** 0.25 for phi in waveform1])]
    ...
    def Hdr(self, amplitudes, durations, starts, phases, freqs, fluxes):
        ...
        Hdr1 = self._tr1.Hdr(amplitudes[0], durations[0], starts[0],
                             phases[0], freqs[0], fluxes[0])
        Hdr1 = [self.two_qubit_operator(qubit1_operator=Hdr1[0]), Hdr1[1]]

        # Hdr2 = self._tr2.Hdr(amplitudes[1], durations[1], starts[1], phases[1])
        Hdr2 = self._tr2.Hdr(amplitudes[1], durations[1], starts[1],
                             phases[1], freqs[1], fluxes[1])
    ...
    def Hdr_cont_RF_RWA(self, amplitudes):
        return [self.two_qubit_operator(
            qubit1_operator=self._tr1.Hdr_cont_RF_RWA(amplitudes[0]))] + \
               [self.two_qubit_operator(
                   qubit2_operator=self._tr2.Hdr_cont_RF_RWA(
                       amplitudes[1]))]

    def _remove_global_phase(self, state):
        state_full = state.full()
        return state * np.sign(state_full[np.argmax(abs(state_full))])[0]
    ...
    def e_state(self, phi1, phi2, qubit_idx, energy=False):
        ...
        if qubit_idx == 1:
            model_state = self.two_qubit_operator(self._tr1.e_state(),
                                                  self._tr2.g_state())
        elif qubit_idx == 2:
            model_state = self.two_qubit_operator(self._tr1.g_state(),
                                                  self._tr2.e_state())
    ...
    def f_state(self, phi1, phi2, qubit_idx, energy=False):
        ...
        if qubit_idx == 1:
            model_state = self.two_qubit_operator(self._tr1.f_state(),
                                                  self._tr2.g_state())
        elif qubit_idx == 2:
            model_state = self.two_qubit_operator(self._tr1.g_state(),
                                                  self._tr2.f_state())
    ...
    def plot_spectrum(self, phi1s, phi2s, exact_H=True, currents=None):
        ...
        fixed_flux_spectra = []
        if (exact_H):
            for phi1, phi2 in tqdm_notebook(fluxes,
                                            desc='Energy levels calculation'):
                H = self.H(phi1, phi2)
        ...
        fixed_flux_spectra = np.array(fixed_flux_spectra)
        eigenlevels = fixed_flux_spectra.T
        transitions_from_g = eigenlevels - eigenlevels[0]

        self._single_photon_transitions = transitions_from_g[
                                          1:3].T / 2 / np.pi
        self._two_photon_transitions = transitions_from_g[
                                       3:6].T / 2 / np.pi / 2

        if currents is not None:
            plot(currents, transitions_from_g[1:3].T / 2 / np.pi)
            plot(currents, transitions_from_g[3:6].T / 2 / np.pi / 2)
        else:
            plot(phi1s, transitions_from_g[1:3].T / 2 / np.pi)
            plot(phi1s, transitions_from_g[3:6].T / 2 / np.pi / 2)
